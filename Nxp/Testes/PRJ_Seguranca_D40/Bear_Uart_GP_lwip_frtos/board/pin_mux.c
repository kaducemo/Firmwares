...
#define BOARD_SW3_PIN  4U
    ...
    // Configura o pino como GPIO com pull-up
       ...
       PORT_SetPinConfig(BOARD_SW2_PORT, BOARD_SW2_PIN, &sw2_config);
	   // Configura o pino como entrada

       // Configura interrupção no pino (borda de descida)
       PORT_SetPinInterruptConfig(BOARD_SW2_PORT, BOARD_SW2_PIN, kPORT_InterruptFallingEdge);

       NVIC_SetPriority(PORTC_IRQn, configLIBRARY_LOWEST_INTERRUPT_PRIORITY);

       // Limpa qualquer flag pendente
       PORT_ClearPinsInterruptFlags(BOARD_SW2_PORT, 1U << BOARD_SW2_PIN);
	   ...
	   GPIO_PinInit(BOARD_SW2_GPIO, BOARD_SW2_PIN, &gpio_config);
	  ...
	  PORT_SetPinConfig(BOARD_SW3_PORT, BOARD_SW3_PIN, &sw3_config);
	   // Configura o pino como entrada

	  // Configura interrupção no pino (borda de descida)
	  PORT_SetPinInterruptConfig(BOARD_SW3_PORT, BOARD_SW3_PIN, kPORT_InterruptFallingEdge);

	  NVIC_SetPriority(PORTA_IRQn, configLIBRARY_LOWEST_INTERRUPT_PRIORITY);

	  // Limpa qualquer flag pendente
	  PORT_ClearPinsInterruptFlags(BOARD_SW3_PORT, 1U << BOARD_SW3_PIN);

	  // Habilita interrupção no NVIC
	  EnableIRQ(BOARD_SW3_IRQ);

	   gpio_pin_config_t gpio_config2 = {
		   kGPIO_DigitalInput,
		   0,
	   };
	   GPIO_PinInit(BOARD_SW3_GPIO, BOARD_SW3_PIN, &gpio_config2);
