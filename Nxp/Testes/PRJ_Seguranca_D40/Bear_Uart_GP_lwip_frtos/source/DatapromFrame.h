/*
 * DatapromFrame.h
 *
 *  Created on: 1 de set de 2025
 *      Author: carlos.oliveira
 */

#ifndef DATAPROMFRAME_H_
#define DATAPROMFRAME_H_

#include <stdint.h>
#include <stddef.h>
#include <stdlib.h>
#include <stdbool.h>
#include "Utilidades.h"




#define ECDH_SEC_LEN 		65U  //Tamanho do Secredo ECDH formato não compactado
#define HKDF_INFO_LEN		16U // Tamanho do vetor INFO
#define HKDF_SALT_LEN	    11U // Tamanho do vetor  SALT

// Tamanhos padrão para GCM
#define GCM_IV_LEN   		12U
#define GCM_TAG_LEN  		16U
#define AES_KEY_LEN  		16U  // AES-128

#define MSG_BY_SESSION		10U //Diz a quantidade de mensagens até a sessão seja trocada

#define END_OFFSET				1U
#define OP_OFFSET				4U
#define RES_OFFSET				5U
#define B64_ITERADOR_OFFSET		6U
#define B64_TAG_OFFSET			18U
#define B64_DATA_OFFSET			42U

#define LEN_MINIMAL_QNS			7U	//Tamanho do quadro nao seguro minimo (sem dados)
#define LEN_MINIMAL_QS			44U  //Tamanho do quadro nao seguro minimo (sem dados, somente opcode cripto)



typedef struct {
	//Campos comuns
    uint8_t endereco[3]; // Endereco Controlador (AREA/CONTROLADOR/SUBCONTROLADOR)
    uint8_t op; //OPCODE NAO SEGURO

    // Utilizado apenas em Quadros Seguros (QS)
    uint8_t res; // Reservado
    uint64_t iterador; // Cotador de pacotes
    uint8_t tag[GCM_TAG_LEN]; // Tag GCM

    // Utilizado apenas em Quadros Nao Seguros (QNS)
    uint8_t *dados;   // Dados (criptografados ou não)
    uint16_t tamanho; // Tamanho dos dados em bytes

} DP_Frame_t;

uint8_t GeraChecksum(uint8_t *in, uint16_t q);
uint8_t CalculaChecksum(DP_Frame_t *frame);
void LimpaMSB(uint8_t *vet, size_t q);
void SetaMSB(uint8_t *vet, size_t q);
bool ObtemIdControladorDoVetor(uint8_t *end, uint8_t *out);
bool VetorizaIdDoControlador(uint8_t cod, uint8_t rede, uint8_t area, uint8_t *out);

DP_Frame_t *ObtemFrameDoVetor(uint8_t *vet, size_t q, uint8_t *ikm, uint8_t salt_id);

DP_Frame_t *ConstroiFrameQNS(uint8_t *end, uint8_t op, uint8_t *dados, size_t q ); //Constroi Quadro Nao Seguro
DP_Frame_t *ConstroiFrameQS(uint8_t *end, uint8_t *dados, size_t q, uint64_t *iterador, uint8_t *ikm, uint8_t salt_id); // Constroi Quadro Seguro
void DestrutorFrames(DP_Frame_t **frame);
uint8_t *VetorizaQuadro(DP_Frame_t *frame, size_t *qOut);


#endif /* DATAPROMFRAME_H_ */
